...
class derive:
    ...
    def sevenpoint(self, f, x, h=0.000855):
        """
    ...
    def richardson(self, f, x, h=1e-3, N_max=3):
        """
        ...
        R[0, 0] = self.cdd(f, x, h)
        for i in range(N_max-1):
            R[i+1, 0] = self.cdd(f, x, h/(2**(i+1)))
            for j in range(i+1):
                R[i+1, j+1] = (4**(j+1) * R[i+1, j] - R[i, j])/(4**(j+1) - 1)
        return R[N_max-1, N_max-1]

    def cdd(self, f, x, h=1e-8):
        """
...
class integrate:
    ...
    def romberg(self, f, ab, n=1, N_max=5):
        """
        ...
        """
        R = np.zeros(
            (N_max, N_max))  # define a matrix which will contain the approximations
        R[0, 0] = self.trapezoid(f, ab, n)  # initiate the first approximation
        for i in range(N_max-1):
            # fill in the remaining approximations for increasing number of subintervals
            R[i+1, 0] = self.trapezoid(f, ab, (2**(i+1))*n)
            for j in range(i+1):
                # iterate over the previous approximations
                R[i+1, j+1] = (4**(j+1) * R[i+1, j] - R[i, j])/(4**(j+1) - 1)
                # to eliminate successive errors
        return R[N_max-1, N_max-1]

    def trapezoid(self, f, ab, n):
        '''
        ...
        '''
        sub_intvls = self.divide_intvl(ab[0], ab[1], n)  # divide the
        I = 0  # initiate the value of the integral calculation
        for intvl in sub_intvls:
            I = I + (intvl[1]-intvl[0])/2*(f(intvl[0]) +
                                           f(intvl[1]))  # add the evaluation
            # of the trapezoid rule
            # for each subinterval
        return I

    def divide_intvl(self, xL, xR, n):
        """
